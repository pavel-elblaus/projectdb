	...
	get: function (cb){
		...
		fse.pathExists(file_server_crypto, function (err, exists){
			...
			// запрос с github
				...
				const release_server = path.join(process.cwd(), "tmp", "server", process.env.PDB_SERVERNAME, "release");
				const release_backup = path.join(process.cwd(), "tmp", "server", process.env.PDB_SERVERNAME, "release.bak");
				// проверка резервной копии
				fse.readFile(release_backup, function (err, release_prev){
					// в случае если есть файл резервной копии
					if(!err && release_prev.length){
						// перевод буфера в строку
						release_prev = release_prev.toString();
						// сверка версии приложения резерва и текущей
						if(release_prev.split(".")[0] === process.env.PDB_VERSION.split(".")[0]){
							// пропуск поиска новой версии так как была найдена резервная копия, был не успешный запуск (source пакет для последней версии не найден в базе)
							print("Search latest release on GitHub skipped, running previous version from backup");
							// восстановление текущей
							fse.outputFile(release_server, release_prev, function(err){
								if(err) return callback(err);
								// запуск
								check(release_prev);
							});
					...
					// запрос версии на которой ранее запускался сервер
					fse.readFile(release_server, function (err, release_prev){
						// перевод буфера в строку
						release_prev = !err && release_prev.length ? release_prev.toString() : false;
						// поиск новой версии, запуск
						!function (next){
							const release_latest = path.join(os.homedir(), ".projectdb", "lib", `release-${process.env.PDB_VERSION.split(".")[0]}.latest`);
							...
							fse.readJson(release_latest, function (err, latest){
								...
								if(err || ( +latest.time || 0 ) + check_timeout < +new Date() || latest.release == null){
									...
									github("tags?per_page=100", false, function (err, data){
										...
										if(err) return latest && latest.release ? next(latest.release) : callback(err);
							...
							// пропуск действий в случае если имя сервера еще не известно
							...
							// создание файла с последней версией
							if(release_prev === false){
								// сохранение текущей
								fse.outputFile(release_server, release_last, function(err){
									if(err) return callback(err);
									// запуск
									check(release_last);
								});
							...
							// сверка с последней, обновление с резервом
							if(release_prev !== release_last){
								// сохранение текущей
								fse.outputFile(release_server, release_last, function(err){
									if(err) return callback(err);
									// сохранение резервной копии
									fse.outputFile(release_backup, release_prev, function(err){
										if(err) return callback(err);
										// запуск
										check(release_last);
									});
