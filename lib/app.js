...
const _path = require_path("path");
const _fs = require_path("fs");
const _fse = require_path("fs-extra");
const _moment = require_path("moment");

/** запрос временной директории процесса */
global.tmp_path = (function (){
	// очистка заголовка процесса
	process.env.PDB_SERVERNAME = "";
...
/** логер ошибок */
global.log = (function () {
	const _strip_ansi = require_path("strip-ansi");
	const _util = require_path("util");
...
process.uuid = (function (){
	const _os = require_path("os");
	const _crypto = require_path("crypto");
...
module.exports = {
	...
	init: function (cli_serv_conf, cb) {
		...
		global.hash_object = (function (){
			...
			const _hash = require_path("object-hash");
		...
		let question = {
			...
			exec: (function () {
				const _readline = require_path("readline");
		...
		step("start the authorization / initialization process")
			...
			.seq(function (next1) {
				...
				question.exec(cli_serv_conf.servername, "Please enter servername [[33m"+cli_conf.servername+"[39m]: ", function (name) {
			...
			.seq(function (next1) {
				...
				// добыть конфигурацию подключения если нет на диске
					...
					const _io_client = require_path("socket.io-client");
					const _crypto = require_path("crypto");
					...
					step("we begin the process of loading / decrypting the configuration of the connection to the main base")
						...
						.seq(function (next2, host) {
							...
							io.on("connect", function(){
								...
								io.binary(false).emit("server.config", process.env.npm_package_version, cli_conf.servername, function (err, buffer) {
			...
			.seq(function (next1, source) {
				...
				if(source === true){
					...
					log.infPrint("source [33m" + process.env.npm_package_version + "[39m version");
					return next1();
				}
				...
				// версия сервера
				process.env.npm_package_version = source.version;
				log.infPrint("source [33m" + process.env.npm_package_version + "[39m version");
				...
				global.require_source = (function (next){
					...
					const _unzip = require_path("unzip-stream");
					...
					return function (path, format = "js") {
						...
						if(format === "js"){
							...
							try {
								// пробуем загрузить
								return require_path(path_module)
							} catch (e) {
								...
								// загружаем модуль
								let module = require_path(path_module);
	...
	run: function (app_server_host){
		...
		if(process.argv[2]){
			let file_cli_serv_conf = _path.join(process.cwd(), "tmp", "server", process.argv[2], "cli.json");
			...
			// фиксируем имя сервера
			cli_serv_conf.servername = process.argv[2];
		}
		...
		this.init(cli_serv_conf, function (next, question) {
			...
			step("command line interface initialization")
				...
				.seq(function (next1) {
					...
					const si = require_path("systeminformation");
