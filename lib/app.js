	...
	get: function (cb){
		...
		fse.pathExists(file_server_crypto, function (err, exists){
			...
			// запрос с github
				...
				// проверка резервной копии предыдущей используемой версии приложения
				const release_server = path.join(process.cwd(), "tmp", "server", process.env.PDB_SERVERNAME, "release");
				fse.readFile(release_server, function (err, release){
					// в случае если есть файл резервной копии
					if(!err && release.length){
						...
						if(err || ( +latest.time || 0 ) + check_timeout < +new Date() || latest.release == null){
							...
							github("tags?per_page=100", false, function (err, data){
								...
								!function (next){
									...
									if(latest && latest.release !== tags[0]){
										// сохранение текущей
										fse.outputFile(release_server, latest.release, function(err){
											if(err) return callback(err);
											// переход к запуску
											next();
										});
