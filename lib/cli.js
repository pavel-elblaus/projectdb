...
const { pathExists, outputJson, ensureDirSync } = require("fs-extra");
...
// функция проверки существования и установки домашней директории
...
program
	...
	.action(function (servername, options) {
		// установка рабочей папки
		set_home_dir_app(options.workPath);
		// конфигурационный файл подключения к серверу ключей для процесса
		let servername_config = join(process.cwd(), "tmp", "server", servername, "cli.json");
		// если переданы логин и пароль, перезаписать конфигурацию
		...
		// поиск конфигурации на диске
			...
			pathExists(database_config, function (err, exists){
				...
				pathExists(servername_config, function (err, exists){
					...
					// ошибка, данные для авторизации не найдены
