...
const { chmod, pathExists, outputFile, outputJson, ensureDirSync, readJson, remove } = require("fs-extra");
...
let set_home_dir_app = function (path){
	try{
		...
		// продолжение невозможно без доступной рабочей директории
...
let question = function (readline, text, default_value, parse, callback){
	...
	readline.question("\x1b[36m" + label + "\x1b[0m " + prompt + suffix + ": ", function (answer){
		...
		// функция parse одновременно преобразует и проверяет ответ
		...
		// передача проверенного значения следующему шагу опросника
		callback(value);
	});
};
...
let output_secure_json = function (path, data, callback){
	...
	outputJson(path, data, { spaces: "\t", mode: 0o600 }, function (err){
		...
		// повторная установка прав нужна и для уже существующего файла
		chmod(path, 0o600, function (err){
			if(err) return exit(err);
			// продолжение только после записи данных и установки прав
			callback();
		});
	});
};
...
let config_exists = function (path, callback){
	...
	pathExists(path, function (err, exists){
		...
		readJson(path, function (err, data){
			...
			// файл прошел проверку формата и может быть выбран командой
			callback(true);
		});
	});
};
...
let parse_integer = function (name, min, max){
	return function (value){
		// преобразование строкового ответа в число
...
let server_key = function (servername){
	// единый набор символов исключает разделители путей и управляющие символы
		...
		let next = function (index){
			if(index < questions.length){
				...
				return question(readline, item.text, item.value, item.parse, function (value){
					// ключ вопроса соответствует полю создаваемой конфигурации
...
Create ${basename(database_config)}?`;
			...
			question(readline, summary, "y", parse_yes_no, function (confirmed){
				// интерфейс больше не нужен независимо от ответа пользователя
				readline.close();
				...
				output_secure_json(database_config, config, function (){
					...
					// возврат к выполнению исходной команды
					callback();
				});
			});
		};
...
let prepare_server_config = function (servername, options, callback){
	// отдельная локальная конфигурация имеет наивысший приоритет
	...
	config_exists(process_database_config, function (exists){
		...
		config_exists(common_database_config, function (exists){
			if(exists) return callback();
			// cli.json с параметрами удаленного сервера рассматривается только после локальных файлов
			...
			if(options.host != null && options.password != null){
				return output_secure_json(servername_config, { host: options.host, password: options.password }, function (){
					// запуск продолжается после полной записи файла
					callback();
				});
			}
...
let get_service = function (servername){
	// одно значение имени используется для systemctl и пути unit-файла
...
let systemd_arg = function (value){
	// двойные кавычки сохраняют пробелы в одном аргументе ExecStart
	// JSON.stringify экранирует кавычки, обратные слеши и управляющие символы
	// символы процента и доллара удваиваются, чтобы systemd не подставлял свои значения
...
let systemctl = function (args, callback){
	...
	execFile("systemctl", args, function (err, stdout, stderr){
		...
		// продолжение цепочки только после успешного завершения systemctl
		callback();
	});
};
...
let connect_existing_pm2 = function (callback){
	// pingDaemon проверяет только существующий демон и, в отличие от connect, не запускает новый
	const pm2 = require("pm2");
	pm2.Client.pingDaemon(function (alive){
		if(!alive) return callback();
		// после успешной проверки используем обычное публичное подключение PM2
...
let stop_pm2_process = function (servername, kill_empty_daemon, callback){
	// проверка не создает демон, если PM2 ранее не использовался
	connect_existing_pm2(function (pm2){
		if(pm2 == null){
			// при выключенном демоне достаточно исключить процесс из основного dump
			let path = get_pm2_dump_path();
			return readJson(path, function (err, proc){
				// без сохраненного списка удалять нечего
				if(err && err.code === "ENOENT") return callback(false);
				if(err) return exit(new Error("Invalid PM2 process list \"" + path + "\": " + err.message));
				if(!Array.isArray(proc))
					return exit(new Error("Invalid PM2 process list \"" + path + "\": an array is required"));
				// резервный dump остается под управлением самого PM2
				let filtered = proc.filter(item => item == null || item.name !== servername);
				if(filtered.length === proc.length) return callback(false);
		...
		// describe ищет точное имя и не затрагивает процессы с похожими именами
		pm2.describe(servername, function (err, proc){
			if(err) return exit(err);
			let active = Array.isArray(proc) && proc.some(item => item && item.name === servername);
			// после проверки или удаления процесса решаем, можно ли остановить пустой демон
			let finish = function (stopped){
				pm2.list(function (err, current){
					if(err) return exit(err);
					if(kill_empty_daemon && current.length === 0){
						// пустой God Daemon останавливается вместе с PM2 Agent
					...
					// callback может продолжить текущую программу, поэтому соединение нужно закрыть
			...
			// публичный API PM2 удаляет зарегистрированное приложение по servername
			pm2.delete(servername, function (err){
				if(err) return exit(err);
				print("Stopped " + servername + " on process manager");
				// PM2 сохраняет новый основной dump стандартным способом
...
program
	...
	.action(function (servername, options) {
		...
		prepare_server_config(servername, options, function (){
			...
			let stop_service = process.env.PDB_METRIC === "service"
				? callback => callback()
			...
			stop_service(function (){
				...
				stop_pm2_process(servername, true, function (){
...
program
	...
	.option("-p, --password <password>", "# config password")
	.action(function (servername, options) {
		// проверка доступности systemd до работы с конфигурацией и файлами
...
program
	...
	.action(function (servername) {
		// проверка доступности systemd до поиска установленного сервиса
		if(process.platform !== "linux")
...
program
	...
	.action(function (servername) {
		// проверка доступности systemd до поиска unit-файла
		if(process.platform !== "linux")
...
program
	...
	.action(function (servername, options) {
		// создание и установка рабочей папки приложения
		set_home_dir_app(options.workPath);
		// эта команда вправе создать демон PM2, поэтому использует обычное подключение
		const pm2 = require("pm2");
		// подготовка функции запуска после конфигурации и устранения конфликтов
		let start_process = function (){
			// обычный connect запускает демон PM2, если он еще не работает
			pm2.connect(function(err) {
				if(err) return exit(err);
		...
		prepare_server_config(servername, options, function (){
			...
			stop_systemd_service(servername, true, function (){
				...
				pm2.agentInfos(function (err, info){
					...
					// PM2 5.4.2 сообщает часть ошибок link через exitCli, а не через callback
...
program
	...
	.action(function (servername) {
		...
		connect_existing_pm2(function (pm2){
			...
			// describe проверяет точное имя до отключения возможного systemd-сервиса
			pm2.describe(servername, function (err, proc){
				if(err) return exit(err);
				...
				stop_systemd_service(servername, true, function (){
					// публичный API PM2 выполняет перезапуск зарегистрированного servername
...
program
	...
	.action(function (servername) {
		...
		if(servername == null){
			// сначала сохраняем пустой dump без подключения и запуска PM2
			let dump_path = get_pm2_dump_path();
			outputFile(dump_path, "[]", { encoding: "utf8", mode: 0o600 }, function (err){
				if(err) return exit(err);
				print("List process saved");
				// подключение к демону выполняется только если он уже запущен
				connect_existing_pm2(function (connected_pm2){
					// при отсутствии демона список уже очищен и команда завершена
					if(connected_pm2 == null) return exit();
					// killDaemon самостоятельно останавливает все процессы и PM2 Agent
					connected_pm2.killDaemon(function (err){
						if(err) return exit(err);
						print("Kill daemon");
						exit();
					});
		...
		stop_pm2_process(servername, true, function (stopped){
			// сообщаем об отсутствии точного имени после проверки существующего демона
			if(!stopped) return exit(new Error("PM2 process " + servername + " does not exist"));
			// процесс, dump и пустой демон уже обработаны общей функцией
			exit();
		});
	});
