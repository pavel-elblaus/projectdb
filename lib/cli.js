...
const { chmod, pathExists, outputFile, outputJson, ensureDirSync, readJson, readFile, remove, readFileSync } = require("fs-extra");
	...
	connect_existing_pm2(function (pm2){
		if(pm2 == null){
			...
			return readFile(path, "utf8", function (err, data){
				// Отсутствующий или пустой файл означает, что сохранённых приложений нет.
				if(err && err.code !== "ENOENT")
					return exit(new Error("Could not read PM2 process list \"" + path + "\": " + err.message));
				let proc = [];
				if(!err && data.trim() !== ""){
					// Непустой повреждённый JSON нельзя принимать за пустой список и затирать.
					try { proc = JSON.parse(data.replace(/^\uFEFF/, "")); }
					catch(parseError){
						return exit(new Error("Invalid PM2 process list \"" + path + "\": " + parseError.message));
					}
				}
