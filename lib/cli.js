...
let exit = function (err){
	...
	if(err)
		console.error("\x1b[31m[PDB][ERROR]\x1b[0m", err instanceof Error ? err.message : _util.inspect(err, { compact: false, colors: true, depth: null }));
	// выход из приложения
	process.exit();
}
	...
	.action(function (servername, options) {
		...
		let callback = function (){
			...
			_pm2.connect(function(err) {
				...
				_pm2.start({
					name: servername,
					script: _path.join(__dirname, "app.js"),
