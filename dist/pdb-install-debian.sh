#!/bin/bash
# Установщик ProjectDB для Debian
...
# Проверка поддерживаемой ОС — Debian 12, 13
if [ "${ID:-}" != debian ] || { [ "${VERSION_ID:-}:$codename" != "13:trixie" ] && [ "${VERSION_ID:-}:$codename" != "12:bookworm" ]; }; then
...
add_php_repository() {
  ...
  curl -fsS https://packages.sury.org/php/apt.gpg | gpg --batch --yes --dearmor | tee $gpg/packages.sury.org.gpg > /dev/null
}
